...
export function BrandpayCheckoutPage() {
  ...
  async function requestPayment() {
    ...
    await brandpay.requestPayment({
      amount: {
        currency: "KRW",
        value: 1, // 여기에는 나중에 SpringBoot 에서 상품 가격이 들어간다.
      },
      orderId: generateRandomString(), // 고유 주문번호
      orderName: "토스 티셔츠 외 2건", // 결제할 때 결제 품목을 DB 에서 전달받을 것
      successUrl: window.location.origin + `/brandpay/success?customerKey=${customerKey}&`, // 결제 요청이 성공하면 리다이렉트되는 URL
      failUrl: window.location.origin + "/fail", // 결제 요청이 실패하면 리다이렉트되는 URL
      customerEmail: "customer123@gmail.com", // 구매자(고객) 이메일 주소
      customerName: "김토스", // 구매자(고객) 이름. 나중에 SpringBoot 와 연결하여 DB 에서 값을 전달받을 것이다.
    });
  }
